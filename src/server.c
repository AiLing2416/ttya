...
static void print_help() {
  // clang-format off
  fprintf(stderr, "ttya is a tool for sharing terminal over the web\n\n"
          "USAGE:\n"
          ...
          "    -H, --auth-header       HTTP Header name for auth proxy, this will configure ttya to let a HTTP reverse proxy handle authentication\n"
...
static int parse_int(char* name, char* str) {
  ...
  if (errno != 0 || endptr == str) {
    fprintf(stderr, "ttya: missing start command\n");
    exit(EXIT_FAILURE);
  }
  return (int)val;
}
...
int main(int argc, char** argv) {
  ...
  if (config_path) {
    parse_config_file(config_path, server, &info, iface, (start == argc));
  }
  ...
  while ((c = getopt_long(start, argv, opt_string, options, NULL)) != -1) {
    switch (c) {
      ...
      case 'p':
        ...
        if (info.port < 0) {
          fprintf(stderr, "ttya: invalid port: %s\n", optarg);
          return -1;
        }
        break;
      case 'i':
      ...
      case 's': {
        ...
        if (sig > 0) {
          ...
          fprintf(stderr, "ttya: invalid signal: %s\n", optarg);
          return -1;
        }
      } break;
      case 'w':
...
#undef sc
      ...
      case 'P': {
        ...
        if (interval < 0) {
          fprintf(stderr, "ttya: invalid ping interval: %s\n", optarg);
          return -1;
        }
      ...
      case 'f': {
        ...
        if (serv_buf_size < 0) {
          fprintf(stderr, "ttya: invalid srv-buf-size: %s\n", optarg);
          return -1;
        }
      ...
      case 't':
        ...
        for (; optind < start && *argv[optind] != '-'; optind++) {
          ...
          if (key == NULL) {
            fprintf(stderr, "ttya: invalid client option: %s, format: key=value\n", optarg);
            return -1;
          }
        ...
        break;
      case 'L':
        strncpy(server->title, optarg, sizeof(server->title) - 1);
  ...
  json_object_object_add(client_prefs, "title", json_object_new_string(server->title));
  json_object_object_add(client_prefs, "writable", json_object_new_boolean(server->writable));
  ...
  lwsl_notice("ttya %s (libwebsockets %s)\n", TTYA_VERSION, lws_get_library_version());
  print_config();
