ttya 1 "September 2016" ttya "User Manual"
==================================================
...
# DESCRIPTION
  ttya is a command-line tool for sharing terminal over the web that runs in *nix and windows systems, with the following features:
...
# OPTIONS
  ...
  -i, --interface <interface>
      Network interface to bind (eg: eth0), or UNIX domain socket path (eg: /var/run/ttya.sock)
  ...
  -H, --auth-header <name>
      HTTP Header name for auth proxy, this will configure ttya to let a HTTP reverse proxy handle authentication
...
# CLIENT OPTIONS
ttya has a mechanism to pass server side command-line arguments to the browser page which is called **client options**:
...
```bash
ttya -t cursorStyle=bar -t lineHeight=1.5 -t 'theme={"background": "green"}' bash
```

# EXAMPLES
  ttya starts web server at port 7681 by default, you can use the -p option to change it, the command will be started with arguments as options. For example, run:
...
```
  ...
  - If you want to login with your system accounts on the web browser, run `ttya login`.
  - You can even run a non-shell command like vim, try: `ttya vim`, the web browser will show you a vim editor.
  - Sharing single process with multiple clients: `ttya tmux new -A -s ttya vim`, run `tmux new -A -s ttya` to connect to the tmux session from terminal.
...
```
ttya --ssl --ssl-cert server.crt --ssl-key server.key --ssl-ca ca.crt bash
```
...
# Docker and ttya
  Docker containers are jailed environments which are more secure, this is useful for protecting the host system, you may use ttya with docker like this:

  - Sharing single docker container with multiple clients: docker run -it --rm -p 7681:7681 tsl0922/ttya.
  - Creating new docker container for each client: ttya docker run -it --rm ubuntu.

# Nginx reverse proxy

Sample config to proxy ttya under the `/ttya` path:
...
# AUTHOR
  Shuanglei Tao \<tsl0922@gmail.com\> Visit https://github.com/tsl0922/ttya to get more information and report bugs.
