...
export class Xterm {
    ...
    constructor(
        ...
        private onWritable: (writable: boolean) => void
    ) { }
    ...
    public async refreshToken() {
        try {
            ...
            console.error(`[ttya] fetch ${this.options.tokenUrl}: `, e);
        }
    }
    ...
    private onSocketClose(event: CloseEvent) {
        console.log(`[ttya] websocket connection closed with code: ${event.code}`);
        ...
        for (const [k, queryVal] of queryObj) {
            ...
            switch (typeof v) {
                ...
                case 'object':
                    ...
                    break;
                default:
                    console.warn(`[ttya] maybe unknown option: ${k}=${queryVal}, treating as string`);
                    prefs[k] = queryVal;
                    break;
            }
        }
    ...
    private onSocketData(event: MessageEvent) {
        ...
        switch (cmd) {
            ...
            case Command.SET_PREFERENCES:
                ...
                break;
            default:
                console.warn(`[ttya] unknown command: ${cmd}`);
                break;
        }
    }
            ...
            switch (key) {
                ...
                case 'disableLeaveAlert':
                    if (value) {
                        ...
                        console.log('[ttya] Leave site alert disabled');
                    }
                    break;
                case 'disableResizeOverlay':
                    if (value) {
                        console.log('[ttya] Resize overlay disabled');
                        this.resizeOverlay = false;
                    }
                    break;
                case 'disableReconnect':
                    if (value) {
                        console.log('[ttya] Reconnect disabled');
                        this.reconnect = false;
                        this.doReconnect = false;
                    }
                    break;
                case 'enableZmodem':
                    if (value) console.log('[ttya] Zmodem enabled');
                    break;
                case 'enableTrzsz':
                    if (value) console.log('[ttya] trzsz enabled');
                    break;
                case 'trzszDragInitTimeout':
                    if (value) console.log(`[ttya] trzsz drag init timeout: ${value}`);
                    break;
                case 'enableSixel':
                    if (value) {
                        terminal.loadAddon(register(new ImageAddon()));
                        console.log('[ttya] Sixel enabled');
                    }
                    break;
                case 'closeOnDisconnect':
                    if (value) {
                        console.log('[ttya] close on disconnect enabled (Reconnect disabled)');
                ...
                case 'titleFixed':
                    ...
                    console.log(`[ttya] setting fixed title: ${value}`);
                    this.titleFixed = value;
                    document.title = value;
                    break;
                case 'isWindows':
                    if (value) console.log('[ttya] is windows');
                    break;
                case 'unicodeVersion':
                    switch (value) {
                        case 6:
                        case '6':
                            console.log('[ttya] setting Unicode version: 6');
                            break;
                        case 11:
                        case '11':
                        default:
                            console.log('[ttya] setting Unicode version: 11');
                    ...
                    break;
                default:
                    console.log(`[ttya] option: ${key}=${JSON.stringify(value)}`);
    ...
    private setRendererType(value: RendererType) {
        ...
        const enableCanvasRenderer = () => {
            ...
            try {
                ...
                console.log('[ttya] canvas renderer loaded');
            } catch (e) {
                console.log('[ttya] canvas renderer could not be loaded, falling back to dom renderer', e);
        ...
        const enableWebglRenderer = () => {
            ...
            try {
                ...
                console.log('[ttya] WebGL renderer loaded');
            } catch (e) {
                console.log('[ttya] WebGL renderer could not be loaded, falling back to canvas renderer', e);
                disposeWebglRenderer();
        ...
        switch (value) {
            ...
            case 'dom':
                disposeWebglRenderer();
                disposeCanvasRenderer();
                console.log('[ttya] dom renderer loaded');
                break;
            default:
                break;
        }
